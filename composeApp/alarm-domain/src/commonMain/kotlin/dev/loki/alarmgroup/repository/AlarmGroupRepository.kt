package dev.loki.alarmgroup.repository

import dev.loki.alarmgroup.model.AlarmGroup
import dev.loki.alarmgroup.model.AlarmMainSort
import dev.loki.alarmgroup.model.TempAlarmGroup
import kotlinx.coroutines.flow.Flow

interface AlarmGroupRepository {

    fun getAlarmGroups(sort: AlarmMainSort): Flow<List<AlarmGroup>> // 알람 그룹 목록 조회
    suspend fun createAlarmGroup(alarmGroup: AlarmGroup)
    suspend fun updateAlarmGroup(alarmGroup: AlarmGroup)
    suspend fun deleteAlarmGroup(alarmGroup: AlarmGroup)
    suspend fun deleteSelectedAlarmGroups(alarmGroups: List<AlarmGroup>)

    fun getTempAlarmGroups(): Flow<List<TempAlarmGroup>> // 임시 저장된 알람 그룹 목록 조회
    suspend fun createTempAlarmGroup(tempAlarmGroup: TempAlarmGroup)
    suspend fun updateTempAlarmGroup(tempAlarmGroup: TempAlarmGroup)
    suspend fun deleteTempAlarmGroup(tempAlarmGroup: TempAlarmGroup)
    suspend fun deleteSelectedTempAlarmGroups(tempAlarmGroups: List<TempAlarmGroup>)
}